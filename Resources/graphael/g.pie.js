...
(function () {
    ...
    function Piechart(paper, cx, cy, r, values, opts) {
        ...
        if (len == 1) {
            series.push(paper.circle(cx, cy, r).attr({ opacity: .6, fill: opts.colors[0], stroke: opts.stroke || "#fff", "stroke-width": opts.strokewidth == null ? 0 : opts.strokewidth }));
            ...
            for (i = 0; i < len; i++) {
                ...
                var p = paper.path(opts.init ? ipath : path).attr({ opacity: .6, fill: opts.colors && opts.colors[values[i].order] || chartinst.colors[i] || "#666", stroke: opts.stroke || opts.colors[values[i].order], "stroke-width": (opts.strokewidth == null ? 0 : opts.strokewidth), "stroke-linejoin": "round" });
